        ...
        if( isset($_POST['login'], $_POST['pwd'] ) && !empty($_POST['login']) && !empty($_POST['pwd']) ) {
            ...
            if(  ctype_alnum($_POST['login']) ) {
                ...
                if( strlen($_POST['pwd']) >= 8  && strlen($_POST['pwd']) <= 20 ) {
                    ...
                    require_once(dirname(__DIR__) . "/_db/dataBase.php");
                    $DBPath = dirname(__DIR__) . "/_db/database.json";
...
<body>
    ...
    <br>
        <form method="post">
            <input type="text" name="login" placeholder="Identifiant" aria-label="Identifiant"<?php if (isset($_GET['login'])) echo ' value="' . $_GET['login'] . '"'; ?>><br><br>
            <input type="password" name="pwd" placeholder="Mot de passe" aria-label="Mot de passe"><br><br>
            <input type="submit" value="Se connecter">
        </form>
        <a href="register.php">Pas encore inscrit ?</a>
        <a href="fogotPwd.php">Mot de passe oublié ?</a>
...
<?php
    ...
    ?>
        <!-- à revoir --> 
        <a href="../admin/cardsAdminPanel.php" >Panneau administration</a>
